...
class Throughput(Realm):
    ...
    def monitor(self, iteration, individual_df, device_names, incremental_capacity_list, overall_start_time, overall_end_time, is_device_configured):
        ...
        for j in range(len(self.angle_list)):
            ...
            while datetime.now() < end_time:
                ...
                if self.dowebgui:
                    ...
                    # Storing individual device throughput data(download, upload, Rx % drop , Tx % drop) to dataframe
                    for i in range(len(download_throughput)):
                        individual_df_data.extend([download_throughput[i], upload_throughput[i], drop_a_per[i], drop_b_per[i], avg_rtt[i]
                                                  [0], int(signal_list[i]), link_speed_list[i], rx_rate_list[i], bssid_list[i]])
        ...
        if self.dowebgui:
            individual_df_for_webui.loc[len(individual_df_for_webui)] = individual_df_data
        # In case of robot testing storing csv for each point
            if self.robo_ip:
                if self.group_name:
                    individual_df_for_webui.to_csv('{}/{}_overall_throughput.csv'.format(runtime_dir, self.current_coordinate), index=False)
    ...
    def generate_report(self, iterations_before_test_stopped_by_user, incremental_capacity_list, data=None, data1=None, report_path='', result_dir_name='Throughput_Test_report',
                        selected_real_clients_names=None, coordinate_list=None, angle_list=None, iot_summary=None):
        ...
        if self.do_interopability is False:
            ...
            # Build average throughput per angle line graph across all completed coordinates when rotation is enabled
            throughput_map = {}
            if self.rotation_enabled:
                for i, coordinate in enumerate(self.coordinates_completed):
                    ...
                    for angle in self.angle_list:
                        if angle == self.current_angle and i == len(self.coordinates_completed):
                            break
                        angle = float(angle)
                ...
                for angle in self.angle_list:
                    ...
                    for i in range(len(iterations_before_test_stopped_by_user)):
                        ...
                        if not self.rotation_enabled:
                            ...
                            report.move_graph_image()
                            report.build_graph()
                        if self.dowebgui and self.get_live_view and not self.robo_ip:
                ...
                for angle in self.angle_list:
                    ...
                    if self.robo_ip:
                        if self.rotation_enabled and coordinate == self.coordinates_completed[len(self.coordinates_completed) - 1] and (self.current_angle == angle):
                            break
                if self.dowebgui and self.get_live_view and self:
                    self.add_live_view_images_to_report(report)
...
''')
    ...
    # Initializing variables related to robot test
    # - rotation_enabled: indicates whether rotation behavior is enabled
    # - angle_list: stores rotation angles provided via command-line arguments
    ...
    for index in range(len(loads_data)):
        ...
        # When Web UI is enabled, generate an empty nav_data.json file and initialize abort and other variables related to robot class
        if args.coordinate:
            abort = False
        ...
        # If no coordinates are given, default to [0] so the test runs once
        # like a standard throughput test.
        for coord in throughput.coordinate_list:
            ...
            # Extend the results dataframe with an Angle column when rotation values are provided
            if args.rotation:
    ...
    if args.dowebgui and args.robot_ip:
