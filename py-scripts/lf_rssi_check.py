...
./lf_rssi_check.py --mgr localhost --endp_type 'lf_udp lf_tcp' --upstream_port 1.1.eth1 \
...
./lf_rssi_check.py --test_duration 5m --polling_interval 1s --upstream_port eth2 \
...
Example using upsteam eth1 downstream eth2
    ./lf_rssi_check.py --test_duration 20s --polling_interval 1s --upstream_port eth1 --downstream_port eth2
...
Example using wifi_settings
    ./lf_rssi_check.py  --lfmgr 192.168.100.116 --local_lf_report_dir
     ...
     --test_rig CT-US-001 --test_tag 'lf_rssi_check'

Example : Have the stations continue to run after the completion of the script
    ./lf_rssi_check.py --lfmgr 192.168.0.101 --endp_type 'lf_udp,lf_tcp' --tos BK --upstream_port 1.1.eth2
        ...
        for endp_name in endp_list['endpoint']:
            if endp_name != 'uri' and endp_name != 'handler':
                for item, endp_value in endp_name.items():
                    # if using existing stations list then the endpoints were existing
        ...
        for ul, dl in itertools.zip_longest(self.side_a_min_rate, self.side_b_min_rate, fillvalue=256000):
            ...
            for ul_pdu, dl_pdu in itertools.zip_longest(self.side_a_min_pdu, self.side_b_min_pdu, fillvalue='AUTO'):
                ...
                for vap in self.vap_list:
                    ...
                    if vap != 'NA':
                        shelf, resource, vap_name, *nil = LFUtils.name_to_eid(vap)
                        url = f"/port/{shelf}/{resource}/{vap_name}"

                        # read LANforge to get the parent device
                        vap_port_info = self.json_get(url)
                        logger.info(pformat(vap_port_info))

                        vap_port_data = vap_port_info['interface']
                        vap_radio_name = vap_port_data['parent dev']
...
-----------------------
./lf_rssi_check.py --mgr <ip_address> --test_duration <duration> --endp_type <traffic types> --upstream_port <port>
...
lf_rssi_check.py --lfmgr LF_MGR_IP --test_duration 90s --polling_interval 10s --upstream_port eth2 \
...
python3 ./lf_rssi_check.py --lfmgr 192.168.0.102 --test_duration 30s --endp_type "lf_tcp lf_udp" --tos "BK VI" --upstream_port 1.1.eth2
...
./lf_rssi_check.py --lfmgr 192.168.100.116 --local_lf_report_dir /home/lanforge/html-reports/ --test_duration 15s
...
--endp_type lf_udp --rates_are_totals --side_a_min_bps=20000 --side_b_min_bps=300000000 --test_rig CT-US-001 --test_tag 'lf_rssi_check'
 ...
 ** FURTHER INFORMATION **
    ...
    parser.add_argument(
        ...
        help="the name of the directory that contains the output from the test /lanforge/html-reports/<results_dir_name> default: lf_rssi_check")
    ...
    parser.add_argument('--sta_start_offset', help='Station start offset for building stations',
                        default='0')
    # TODO for now would like to not perform a pre clean up
    parser.add_argument('--no_pre_cleanup', help='Do not pre cleanup stations on start',
                        action='store_false')
    # TODO for now would like to not perform a clean up
    ...
    if args.csv_outfile is not None:
        ...
        csv_outfile = "{}_{}-lf_rssi_check.csv".format(
    ...
    report.set_title("Test Layer 3 Cross-Connect Traffic: lf_rssi_check.py ")
