...
TEST_CFG=TR-398_Issue_2.cfg                 # 'TR-398 Issue 2' test config file. If not specified in test script
                                            # CLI, then any unspecified options will use default test values.

RPT_DIR=/tmp/TR-398_Issue_2_reports         # Output directory for generated reports and associated data

# Chamber View Scenario name
CV_SCENARIO_NAME=TR-398_Issue_2_Automated_Test
...
DUT_NAME=TR-398_Issue_2_DUT                 # Name of DUT as it appears in report
...
# 3. RUN 'TR-398 Issue 2' TEST
#
# NOTE: The '--dut6' arg is not supported on LANforge pre-5.4.7, so use the '--raw_line' argument to set it.
#
# See README in same directory for instructions on building a
# Chamber View scenario with the 'lf_tr398v2_test.py' script.
printf "Starting TR-398 Issue 2 test"

$LF_PY_SCRIPTS/lf_tr398v2_test.py \
    --mgr                 $MGR \
    --port                $MGR_PORT \
    --lf_user             "lanforge" \
