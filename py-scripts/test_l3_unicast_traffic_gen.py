...
NOTES:
This script is not functional : consider moving to scripts deprecated
Look at lanforge_api.py   EAP-PEAP  # Use EAP-PEAP connection logic on all ssids, deprecated, see add_dut_ssid.
    ...
    def start(self, print_pass=False, print_fail=False):
        ...
        for station_profile, station_list in zip(self.station_profiles, self.station_lists):
            ...
            if self.wait_for_ip(station_list=station_list, timeout_sec=10 * len(station_list)):
                if self.debug:
                    logger.info("ips acquired {}".format(station_list))
            else:
