...
class Roam(Realm):
    ...
    def set_attenuators(self):
        logging.info('Setting attenuation to {} for attenuator {}'.format(
            0, self.attenuator1))
        self.set_atten(self.attenuator1, 0)

        logging.info(
            'Setting active attenuator as {}'.format(self.attenuator1))
        self.active_attenuator = self.attenuator1

        logging.info(
            'Setting passive attenuator as {}'.format(self.attenuator2))
        self.passive_attenuator = self.attenuator2

        logging.info('Setting attenuation to {} for attenuator {}'.format(
            self.max_attenuation, self.attenuator2))
        self.set_atten(self.attenuator2, self.max_attenuation)
    ...
    def start_sniff(self, capname='roam_test.pcap'):
        ...
        self.monitor.start_sniff(capname=capname, duration_sec=300)
    ...
    def generate_report(self, result_json=None, result_dir='Roam_Test_Report', report_path=''):
        ...
        total_attempted_roams = len(self.station_list) * self.iterations
        ...
        # graph for above
        graph = lf_bar_graph_horizontal(_data_set=[[total_attempted_roams], [total_successful_roams], [total_failed_roams]],
