...
class L3VariableTime(Realm):
    def __init__(self,
        ...
        for radio_, ssid_, ssid_password_, ssid_security_, wifimode_,reset_port_enable_, reset_port_time_min_, reset_port_time_max_ in zip(radio_name_list, ssid_list, ssid_password_list, ssid_security_list, wifimode_list, reset_port_enable_list, reset_port_time_min_list, reset_port_time_max_list):
    ...
    def __compare_vals(self, old_list, new_list, initial_channel):
        passes = 0
        expected_passes = 0
        csv_rx_headers = []
        csv_rx_row_data = []
        ...
        if len(old_evaluate_list) == len(new_evaluate_list):
            ...
            final_channel = self.cc_module.show_ap_dot11_5gz_summary()["channel"]

            if int(final_channel) != int(initial_channel):
                pass_fail = "pass"
            elif self.cc_module.is_nop_started(initial_channel) == True:
                pass_fail = "pass"
            else:
                logg.info("FAIL: The DFS channel did not change or initial channel was not DFS")
                pass_fail = "fail"
            
            frequency_ = self.dfs_get_frequency(initial_channel)
            ...
            csv_rx_row_data.extend([pass_fail, final_channel])
            if self.rf_dict:
                csv_rx_row_data.extend([self.rf_dict['pulse_width'], self.rf_dict['pulse_interval'], self.rf_dict['pulse_count'], self.rf_dict['sweep_time'], frequency_, self.rf_dict['gain'], self.rf_dict['if_gain'], self.rf_dict['bb_gain']])
    ...
    def build(self):
        ...
        for station_profile in self.station_profiles:
            ...
            for _tos in self.tos:
                print(f"endp_type {self.endp_type} tos {_tos} station names {station_profile.station_names} side b {self.side_b}")
                logg.info(
    ...
    def dfs_send_radar(self, channel):
        ...
        if self.rf_dict:
            width_ = self.rf_dict['pulse_width']
            interval_ = self.rf_dict['pulse_interval']
            count_ = self.rf_dict['pulse_count']
            sweep_time_ = self.rf_dict['sweep_time']
            if_gain_ = self.rf_dict['if_gain']
            bb_gain_ = self.rf_dict['bb_gain']
            gain_ = self.rf_dict['gain']
        else:
            width_ = "4"
            interval_ = "778"
            count_ = "10"
        ...
        # With timeouts
        #################################
        command_hackRF = "sudo python3 lf_hackrf.py --pulse_width {} --pulse_interval {} --pulse_count {} --sweep_time {} --freq {} --if_gain {} --bb_gain {} --gain {}".format(
    ...
    def ap_cac_verify(self):
        if bool(self.ap_dict):
            ...
            logg.info("rf_dict {}".format(self.rf_dict))
            logg.info(
                "Read RF action: {} pulse_width: {} pulse_interval: {} pulse_count: {} sweep_time: {} gain: {} if_gain: {} bb_gain: {}".format(
                    "show_log", self.ap_dict['pulse_width'], self.ap_dict['pulse_interval'], self.ap_dict["pulse_count"],
                    self.ap_dict['sweep_time'], self.ap_dict['gain'], self.ap_dict['if_gain'], self.ap_dict['bb_gain']))
            try:
                self.cc_module.ap_cac_verify()
    ...
    def start(self, print_pass=False, print_fail=False):
        ...
        if self.band == '5g' or self.band == "u5g":
            initial_channel=self.chan_5ghz
        elif self.band == '24g':
            initial_channel=self.chan_24ghz
        ...
        if not dfs_radar_sent:
            if self.dfs:
                # if self.cc_module.band == '5g':
                #     pss = self.cc_module.dfs_event_5ghz()
                #     logg.info(pss)
                # elif self.cc_module.band == '24g':
                #     pss = self.cc_module.dfs_event_24ghz()
                #     logg.info(pss)
        ...
        Result, max_tp_mbps, csv_rx_row_data = self.__compare_vals(old_rx_values, new_rx_values, initial_channel)
        ...
        #blacklist_time = self.read_auto_rf() i don't know what this means
        # blacklist_time = 0
        ...
        # self.csv_add_column_headers_results(best_csv_rx_row_data)
    ...
    def csv_generate_column_headers(self):
        ...
        csv_rx_headers.extend(['max_tp_mbps', 'expected_tp', 'test_id', 'epoch_time', 'time', 'monitor', 'pass_fail', 'final_channel', 'pulse_width', 'pulse_interval', 'pulse_count', 'sweep_time', 'frequency', 'gain', 'if_gain', 'bb_gain'])
        '''for i in range(1,6):
...
Sample script 2/11/2021
   ...
   ./lf_dfs_test.py -cc 192.168.100.112 -cu admin -cpw controller123 -cca APA453.0E7B.CF9C -ccf "a" -cwm "auto" -cc5 "52" -ccw "20" -ccd "1" -cs "3504" --endp_type 'lf_udp' --upstream_port eth2  --controller_wlan "test_candela" --controller_wlanID 1 --controller_wlanSSID "test_candela" --controller_directions "upstream" --controller_prompt "(controller Controller)" --radio "radio==1.wiphy0 stations==1  ssid==test_candela ssid_pw==[BLANK] security==open wifimode==auto" --ap_info "ap_scheme==serial ap_prompt==APA453.0E7B.CF9C ap_ip==0 ap_port==0 ap_user==admin ap_pw==Admin123 ap_tty==/dev/ttyUSB2 ap_baud==9600"
    ...
    parser.add_argument('-ccf', '--controller_band', type=str, help='--controller_band <5g | 24g | u5g> default 5g', default="5g")
    ...
    parser.add_argument('-cde', '--controller_data_encryption', help='--controller_data_encryption \"enable disable\"',
                        default="disable")
    parser.add_argument('-cs', '--controller_series', help='--controller_series <9800 | 3504 | opensync | openwrt>', required=True,
                        choices=["9800", "3504", "opensync", "openwrt"])
    ...
    parser.add_argument('--module', type=str, help='[controller configeration] series module (cc_module_plumemos.py)  --module cc_module_plumeos ', required=True)
    ...
    parser.add_argument('-rf', '--rf_settings', action='append', nargs=1, type=str,
                        help='(enter 0 if does not apply) --rf_settings \"pulse_width==<pulse width> pulse_interval==<pulse interval> pulse_count==<pulse count> sweep_time==<sweep time> gain==<0 or 14db> if_gain==<if-gain> bb_gain==<bb-gain>\" ')
    ...
    rf_dict = []
    if args.rf_settings:
        rf_settings = args.rf_settings
        for _rf_setting in rf_settings:
            print("rf_settings {}".format(_rf_setting))
            rf_keys = ['pulse_width', 'pulse_interval', 'pulse_count', 'sweep_time', 'gain', 'if_gain', 'bb_gain']
            rf_dict = dict(
                map(lambda x: x.split('=='), str(_rf_setting).replace('[', '').replace(']', '').replace("'", "").split()))
            for key in rf_keys:
                if key not in rf_dict:
                    print("missing rf config, for the {}, all thse need to be set {} ".format(key, ap_keys))
                    exit(1)
            print("rf_dict: {}".format(rf_dict))
    ...
    myProduct = itertools.product(
    controller_aps, controller_bands, controller_wifimodes,
    controller_tx_powers, controller_chan_5ghzs, controller_chan_24ghzs,
    controller_chan_widths, controller_data_encryptions, controller_client_densities,
    controller_packet_types, controller_directions, controller_packet_sizes
    )
    for i in myProduct:
        controller_ap = i[0]
        controller_band = i[1]
        controller_wifimode = i[2]
        controller_tx_power = i[3]
        controller_chan_5ghz = i[4]
        controller_chan_24ghz = i[5]
        controller_chan_width = i[6]
        controller_data_encryption = i[7]
        controller_client_density = i[8]
        controller_packet_type = i[9]
        controller_direction = i[10]
        controller_packet_size = i[11]
        ...
        logg.info(
            "#####################################################")
        test_config = "AP=={} Band=={} chan_5ghz=={} chan_24ghz=={} wifi_mode=={} BW=={} encryption=={} clients=={} packet_type=={} direction=={} packet_size=={}".format(
        ...
        if args.no_controller:
            ...
            if (controller_ap != __ap_set or
                ...
                series = importlib.import_module(args.module)
                    # 'ap_scheme', 'ap_prompt', 'ap_ip', 'ap_port', 'ap_user', 'ap_pw'
                controller = series.create_controller_series_object(
                    dest=ap_dict["ap_ip"],
                    user=ap_dict["ap_user"],
                    passwd=ap_dict["ap_pw"],
                    band=__band_set,
                )
                controller.scheme=ap_dict["ap_scheme"]
                controller.prompt=ap_dict["ap_prompt"]
                controller.ap=ap_dict["ap_prompt"],
                controller.series=__series
                controller.wlan=args.controller_wlan
                controller.wlanID=args.controller_wlanID
                controller.wlanSSID=args.controller_wlanSSID
                controller.tx_power=__tx_power_set
                if 'PlumeOS_834-5' in controller.ap:
                    controller.radio_name_24g = "phy1"
                    controller.radio_name_5g = 'phy0'
                if args.controller_series in ['opensync']:
                    controller.jump_host = {
                    "host": __ctlr,
                    "auth_username": args.controller_passwd,
                    "auth_password": args.controller_user
                    }
                controller.console_setup()
                if controller.band == '5g' or controller.band == 'u5g':
                    pss = controller.show_ap_dot11_5gz_summary()
                    logg.info(pss)
                    pss = controller.show_ap_bssid_5ghz()
                    logg.info(pss)
                elif controller.band == '24g':
                    pss = controller.show_ap_dot11_24gz_summary()
                    logg.info(pss)
                    pss = controller.show_ap_bssid_24ghz()
                    logg.info(pss)
                controller.tx_power = controller_tx_power
                controller.bandwidth = controller_chan_width
                if controller.band == '5g' or controller.band == 'u5g':
                    controller.channel = controller_chan_5ghz
                    # pss = controller.config_dot11_5ghz_tx_power()
                    # logg.info(pss)
                    pss = controller.config_dot11_5ghz_channel()
                    logg.info(pss)
                    pss = controller.config_dot11_5ghz_channel_width()
                    logg.info(pss)
                if controller.band == '6g':
                    # pss = controller.config_dot11_6ghz_tx_power()
                    # logg.info(pss)
                    pss = controller.config_dot11_6ghz_channel()
                    logg.info(pss)
                    # pss = controller.config_dot11_6ghz_channel_width()
                    # logg.info(pss)
                elif controller.band == '24g':
                    controller.channel = controller_chan_5ghz
                    pss = controller.controller_chan_24ghz()
                    logg.info(pss)
                    pss = controller.config_dot11_24ghz_channel()
                    logg.info(pss)
                    # pss = controller.config_dot11_24ghz_channel_width()
                    # logg.info(pss)
                    # enable 5g wlan
                    pss = controller.config_no_ap_dot11_5ghz_shutdown()
                    logg.info(pss)
                    # enable 5g operation status
                    pss = controller.config_ap_no_dot11_5ghz_shutdown()
                    logg.info(pss)
                elif controller.band == "24g":
                    # enable wlan 24ghz
                    pss = controller.config_no_ap_dot11_24ghz_shutdown()
                    logg.info(pss)
                    # enable 24ghz operation status
                    controller.config_ap_no_dot11_24ghz_shutdown()
                    logg.info(pss)
                ...
                logg.info(
                    "###############################################")
                logg.info(
                    "controller_ap: {} controller_band: {} controller_chan_width: {} controller_tx_power: {} controller_chan_5ghz: {} controller_chan_24ghz: {}".format(
                ...
                logg.info(
                    "__ap_set: {} __band_set: {} __chan_width_set: {} __tx_power_set: {} __chan_5ghz_set: {} __chan_24ghz_set: {}".format(
                        __ap_set, __band_set,
                        __chan_width_set,
                        __tx_power_set,
                        __chan_5ghz_set, __chan_24ghz_set))
            ...
            time.sleep(3)
            if args.controller_series in ["opensync", "openwrt"]:
                logg.info("smartos read controller dBm")
                if controller.band == '5g' or controller.band == 'u5g':
                    logg.info("show ap dot11 5ghz summary")
                    logg.info(f"ap: {controller.ap} ap_band_slot_5g: {controller.radio_name_5g}")
                    pss = controller.show_ap_dot11_5gz_summary()
                    logg.info(pss)
                else:
                    logg.info("show ap dot11 24ghz summary")
                    logg.info(f"ap: {controller.ap} ap_band_slot_24g: {controller.radio_name_24g} ")
                    pss = controller.show_ap_dot11_24gz_summary()
                    logg.info(pss)
                if args.controller_series == "openwrt":
                    cc_mac = pss["bssid"]
                    cc_ch = pss["channel"]
                    cc_bw = pss["htmode"]
                    cc_power = pss["txpower"]
                    cc_dbm = pss["txpower"]
                else:
                    cc_mac = pss["mac"]
                    cc_ch = pss["channel"]
                    cc_bw = pss["ht_mode"]
                    cc_power = pss["tx_power"]
                    cc_dbm = pss["tx_power"]
                assert(cc_ch)
                logg.info(f"show ap summary : {pss}")
                logg.info(f"smartos test_parameters_summary:  read: tx: {cc_power} ch: {cc_ch} bw: {cc_bw}")
                logg.info(f"smartos from ap summary cc_mac:   read : {cc_mac}")
            ######################################################
            ...
            for radio_ in radios:
                ...
                print(radio_info_dict)
                # print(f"mcather {ssid_password_list}")
                assert ssid_password_list
        ...
        if args.no_stations:
            ...
            ip_var_test = L3VariableTime(
                ...
                _series=controller.series,
                _cc_module=controller,
                _band=__band_set,
                _chan_5ghz=__chan_5ghz_set,
                _chan_24ghz=__chan_24ghz_set,
                _chan_width=controller.bandwidth,
                _tx_power=controller.tx_power,
