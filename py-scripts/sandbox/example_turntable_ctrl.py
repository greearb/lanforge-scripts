...
class Turntable(Realm):
    ...
    def start(self, no_settle=None):
        self.api_session.logger.info("Locating chamber [%s]" % self.chamber_name)
        if not self.locate_chamber(chamber_name=self.chamber_name):
            self.api_session.logger.error("Unable to locate chamber, bye")
            sys.exit(1)
        ...
        self.api_session.logger.info("Setting new chamber position")
        ...
        while now_ms <= until_ms:
            if not self.locate_chamber(self.chamber_name):
                ...
                sys.exit(1)
            self.api_session.logger.info("Position %s dT %s" %
                         (float(self.found_chamber[Turntable.CURRENT_ANGLE]),
                          until_ms - now_ms))
