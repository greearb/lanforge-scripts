...
class IPV4L4(Realm):
    def __init__(self,
        ...
        # create api_json
        self.json_endp_api = lf_json_api.lf_json_api(lf_mgr=self.lfclient_host,
    ...
    # http://localhost:8080/help/set_cx_report_timer
    # cli example:
    # default@btbits>> set_cx_report_timer default_tm CX_sta0000_l4 1000
...
---------------------------
    ...
    test_l4_parser.add_argument('--polling_interval', help="--polling_interval <h m s ms>", default='1000ms')
    ...
    # Test Configuration
    test_l4_parser.add_argument('--polynomial', help="--polynomial store_true , show polynomial lines on retries graph", action="store_true")
    test_l4_parser.add_argument('--interpolate', help="--interpolate store_true , show interpolation on retries graph", action="store_true")
    ...
    polling_interval_milliseconds = ip_test.duration_time_to_milliseconds(args.polling_interval)
    l4endp_id_list = []
    created_l4_cx_list = []
    l4endp_id_list = []
    ...
    if not isinstance(ip_test_json_data, list):
        ...
        l4endp_id_list = ip_test_json_data['name']
        created_l4_cx_list.append(ip_test_json_data['name'])
        layer4endp_link = ip_test_json_data['_links']
        # Set the report timer (use the duration in seconds to set the report timer):
        rpt_timer = polling_interval_milliseconds / 2
        ip_test.set_port_report_timer(port=layer4traffic, milliseconds=int(rpt_timer))

    else:
        # create list for layer4 endp names:
        layer4traffic = ','.join([[*x.keys()][0] for x in ip_test.json_get('layer4')['endpoint']])
        created_l4_cx_list = layer4traffic.split(',')
        logger.info(created_l4_cx_list)
        # create list for layer4 endp _links
        len_test_data = len(ip_test_json_data)
        # polling_interval_milliseconds = ip_test.duration_time_to_milliseconds(args.polling_interval)
        for endps in range(len_test_data):
            logger.info(ip_test_json_data[endps])
            for port in ip_test_json_data[endps]:
                l4endp_data = ip_test_json_data[endps][port]
                # Set the report timer (use the duration in seconds to set the report timer):
                rpt_timer = polling_interval_milliseconds / 2
                ip_test.set_port_report_timer(port=port, milliseconds=int(rpt_timer))
                # ip_test.set_port_report_timer(port=i, milliseconds=args.polling_interval)
                if '_links' in l4endp_data:
                    l4endp_link = l4endp_data['_links']
                    l4endp_link_split = l4endp_link.split('/')
                    l4endp_id_list.append(l4endp_link_split[2])

        # layer4endp_link = ','.join([[*x.keys()][1] for x in ip_test.json_get('layer4')['endpoint']])
    # rpt_timer = args.polling_interval
    # rpt_timer = polling_interval_milliseconds / 2
    ...
    endp_rx_map, endps, total_bytes_rd, total_bytes_wr, total_rx_rate, total_tx_rate, urls_seconds, total_urls = ip_test.get_rx_values()
    ...
    # Time Interval Table
    report.set_table_title("Time Interval")
    report.build_table_title()

    #
    # NOTE: code for time interval and data polling:
    #
    l4_interval = []
    l4_interval_time = []
    l4_total_name = []
    ...
    monitor_interval = args.polling_interval
    duration_sec = args.test_duration
    if 'ms' in monitor_interval:
        pattern = re.split('(\D)',monitor_interval)  # /D: Matches any character which is not a decimal digit.
        polling_interval = int(pattern[0])
        # convert milliseconds to seconds
        monitor_interval = polling_interval / 1000
    else:
        polling_interval = ip_test.duration_time_to_milliseconds(monitor_interval)
        monitor_interval = ip_test.duration_time_to_seconds(monitor_interval)
        # polling_interval = monitor_interval
    # Monitor columns
    sleep_interval = (polling_interval_milliseconds/1000)/2
    interval = 0
    # initialize time stamps
    endp_count = len(l4endp_id_list)
    value_map = dict()
    duration_sec = ip_test.parse_time(args.test_duration)
    l4_endp_list = []
    # for i in range(endp_count):
    cur_time = datetime.datetime.now()
    # end_time = self.parse_time(duration_sec) + cur_time
    end_time = duration_sec + cur_time
    
    # current_time_stamp = json_l4_endp_stats["endpoint"]["time-stamp"]
    # previous_time_stamp = current_time_stamp
    bytes_rd_previous = 0
    urls_previous = 0
    total_bytes_rd_endps = 0
    total_urls_endps = 0
    # bytes_rd_previous = json_l4_endp_stats["endpoint"]["bytes-rd"]
    # urls_previous = json_l4_endp_stats["endpoint"]["total-urls"]
    # clear_endp = "cli-json/clear_endp_counters"
    # data = {
    #         "endp_name": "all"
    # }
    # ip_test.json_post(clear_endp, data)
    # time.sleep(.1)
    for l4_endp_id in l4endp_id_list:
        # json_l4_endp_stats, *nil = ip_test.json_endp_api.get_request_layer4_information(endpoint=l4endp_id_list[l4_endp_count])
        clear_endp = "cli-json/clear_endp_counters"
        data = {
            "endp_name": "all"
        }
        ip_test.json_post(clear_endp, data)
        json_l4_endp_stats, *nil = ip_test.json_endp_api.get_request_layer4_information(endpoint=l4_endp_id)
        bytes_rd_previous = json_l4_endp_stats["endpoint"]["bytes-rd"]
        urls_previous = json_l4_endp_stats["endpoint"]["total-urls"]
    ...
    while cur_time < end_time:
        ...
        logger.info(cur_time)  # test:
        logger.info(end_time)  # test:
        # json_l4_endp_stats, *nil = self.json_endp_api.get_request_layer4_information(endpoint=l4endp_id[i])
        ...
        current_total_urls = 0
        current_total_bytes_rd = 0
        current_total_err = 0
        current_urls_seconds = 0
        ...
        for l4_endp_id in l4endp_id_list:
            # json_l4_endp_stats, *nil = ip_test.json_endp_api.get_request_layer4_information(endpoint=l4endp_id_list[l4_endp_count])
            json_l4_endp_stats, *nil = ip_test.json_endp_api.get_request_layer4_information(endpoint=l4_endp_id)
            logger.info(json_l4_endp_stats)  # test

            logger.debug("json_l4_endp_stats type {endp}".format(endp=type(json_l4_endp_stats)))
            current_time_stamp = json_l4_endp_stats["endpoint"]["time-stamp"]
            current_total_name = json_l4_endp_stats["endpoint"]["name"]
            current_total_err += json_l4_endp_stats["endpoint"]["total-err"]
            current_total_urls += json_l4_endp_stats["endpoint"]["total-urls"]
            current_total_bytes_rd += json_l4_endp_stats["endpoint"]["bytes-rd"]
            current_urls_seconds += json_l4_endp_stats["endpoint"]["urls/s"]
            # have a running average since added the previous rate
            average_rx_rate += json_l4_endp_stats["endpoint"]["rx rate"]
            if first_collection:
                first_collection = False
            else:
                average_rx_rate = average_rx_rate / 2
                ...
                #     logger.info("new TX stats: time_stamp {time} previous_time_stamp {pre_time}".format(time=current_time_stamp, pre_time=previous_time_stamp))  # test
                #     previous_time_stamp = current_time_stamp
                #     break
                # Here check if the time stamp has changed
            # json_l4_endp_stats, *nil = self.json_endp_api.get_request_layer4_information(endpoint=l4endp_id[i])
            interval += float(polling_interval_milliseconds) / 1000
            # l4_interval.append(interval)
            current_time = current_time_stamp
            # l4_interval_time.append(current_time)
            l4_total_name.append(current_total_name)
            l4_total_err.append(current_total_err)
            # l4_total_urls.append(current_total_urls)
        l4_total_bytes_rd.append(current_total_bytes_rd)
        total_bytes_rd += current_total_bytes_rd
        total_urls += current_total_urls


        # l4_total_rx_rate.append(current_total_rx_rate)
        # delta_bytes_rd.append(json_l4_endp_stats["endpoint"]["bytes-rd"] - bytes_rd_previous)
        # delta_urls.append(json_l4_endp_stats["endpoint"]["total-urls"] - urls_previous)
        # bytes_rd_previous = json_l4_endp_stats["endpoint"]["bytes-rd"]
        # urls_previous = json_l4_endp_stats["endpoint"]["total-urls"]
        delta_bytes_rd.append(current_total_bytes_rd - bytes_rd_previous)
        delta_urls.append(current_total_urls - urls_previous)
        
        bytes_rd_previous = current_total_bytes_rd
        urls_previous = current_total_urls
            #rx_retries.append(json_l4_endp_stats["interface"]["wifi retries"] - rx_retries_previous)
            # tx_retries_previous = json_l4_endp_stats["interface"]["wifi retries"]
            # self.tx_failed.append(round(json_l4_endp_stats["interface"]["tx-failed %"], 2))
            # calculated the transmitted packets compared to number of retries
        l4_interval.append(interval)
        l4_interval_time.append(current_time)
            # l4_total_bytes_rd.append(total_bytes_rd_endps)
            # l4_total_urls.append(total_urls_endps)

    
    # NOTE: the first 8-seconds of the test report no data - this can be seen
    #        to occur on the gui as well. appears that the test is spooling up.
    df_l4_time_info = pd.DataFrame({" Time Interval (s) ": [ti for ti in l4_interval],
                                    " Time ": [t for t in l4_interval_time],
                                    # " L4 Endp Name ": [tn for tn in l4_total_name],
                                    # " Total URL's ": [tu for tu in l4_total_urls],
                                    " Delta URL's ": [du for du in delta_urls],
                                    # " Total Bytes Read ": [tb for tb in l4_total_bytes_rd],
                                    " Delta Bytes Read ": [db for db in delta_bytes_rd],
                                    # " Total RX Rate ": [k for k in l4_total_rx_rate],
                                    # " Total Error ": [te for te in l4_total_err]
                                    })
    report.set_table_dataframe(df_l4_time_info)
    report.build_table()
    html_file = csv_results_file
    # lf_bar_line_graph
    # delta_bytes_rd, delta_urls
    if html_file:
        graph = lf_bar_line_graph(
            # _data_set1=[delta_bytes_rd,l4_total_bytes_rd],
            _data_set1=[delta_bytes_rd],
            # _data_set2=[delta_urls],
            _data_set2=[delta_urls],
            ...
            _y1axis_name="Bytes-rd",
            _y2axis_name="URLs",
            # _xaxis_categories=l4_interval,
            _xaxis_categories=l4_interval,
            _graph_image_name="RX Info bar line",
            # _label1=[" Delta Bytes-rd ", " Total Byte-rd "],
            # _label1=[" Total Byte-rd "],
            _label1=[" Delta Byte-rd "],
            _label2=[" Delta URLs"],
            # _label2=[" Total URLs"],
        ...
        report.build_graph()
        # for endp_l4_name in layer4traffic[i]:
        # endp_dict[json_l4_endp_stats["endpoint"]["name"]] = {'polling-interval': l4_interval,
        #                                    'time-stamp': l4_interval_time,
        #                                    'name': current_total_name,
        #                                    'total-err': l4_total_err,
        #                                    'total-urls': l4_total_urls,
        #                                    'bytes-rd': l4_total_bytes_rd,
        #                                    'rx rate': l4_total_rx_rate }
        # l4_endp_list.append(endp_dict)
    # logger.info(endp_dict)  # test
    # logger.info(l4_endp_list)  # test
    # return l4_interval, l4_interval_time, l4_total_name, l4_total_err, l4_total_urls, l4_total_bytes_rd, l4_total_rx_rate, delta_bytes_rd, delta_urls
    # NOTE: end of polling code ^. Start of graphing:
    # NOTE: the first 8-seconds of the test report no data - this can be seen
    #        to occur on the gui as well. appears that the test is spooling up.
    # df_l4_time_info = pd.DataFrame({" Time Interval (s) ": [ti for ti in l4_interval],
    #                                 " Time ": [t for t in l4_interval_time],
    #                                 " L4 Endp Name ": [n for n in l4_total_name],
    #                                 " Total URL's ": [k for k in l4_total_urls],
    #                                 " Total Bytes Read ": [k for k in l4_total_bytes_rd],
    #                                 # " Total RX Rate ": [k for k in l4_total_rx_rate],
    #                                 " Total Error ": [k for k in l4_total_err]})
# 
    # report.set_table_dataframe(df_l4_time_info)
    # report.build_table()
# 
...
# 
    ...
    #     report.build_graph()
# 
    ip_test.record_kpi_csv(temp_stations_list, total_test, total_pass, total_bytes_rd, total_bytes_wr, average_rx_rate,
                           total_tx_rate, urls_seconds, total_urls)
    ip_test.record_results(len(temp_stations_list), total_bytes_rd, total_bytes_wr, average_rx_rate, total_tx_rate,
                           urls_seconds, total_urls)
    # NOTE: results in this table poll data while test is still running after the time-
    #        interval table. the results displayed between tables with currently differ.
