...
if [[ ${#SHORT} -gt 0 ]]; then
  ...
  testCommands=(
      ...
      'radio==$RADIO2 stations==1 ssid==$SSID_USED ssid_pw==$PASSWD_USED security==$SECURITY' --test_duration 5s --rates_are_totals --side_a_min_bps=20000 \
      --side_b_min_bps=300000000  -o longevity.csv --debug"
