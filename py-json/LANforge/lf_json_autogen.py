"""----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    Generated by LANforge JsonApiPythonGenerator, Thu Jun 10 12:54:25 PDT 2021
...
class LFJsonGet(LFCliBase):
    def __init__(lfclient_host='localhost',
             lfclient_port=8080,
             debug_=False,
...
class LFJsonPost(LFCliBase):
    ...
    class add_bgp_peer_flags(Enum):
        ENABLE_PEER               = 1      # Set this to zero if you don't want this peer enabled.
        PEER_CLIENT               = 2      # Sets corresponding Xorp flag in BGP Peer section.
        PEER_CONFED_MEMBER        = 4      # Sets corresponding Xorp flag in BGP Peer section.
        PEER_UNICAST_V4           = 8      # Sets corresponding Xorp flag in BGP Peer section.
    ...
    class add_chamber_chamber_flags(Enum):
        OPEN                   = 4      # (3) Door is open, no real isolation right now.
        PHANTOM                = 1      # (1) Chamber is not actually here right now.
        VIRTUAL                = 2      # (2) No real chamber, open-air grouping of equipment.
    ...
    class add_channel_group_types(Enum):
        clear           = 0      # Channel(s) are bundled into a single span.  No conversion or
        e_m             = 0      # Channel(s) are signalled using E&amp;M signalling (specific
        fcshdlc         = 0      # The zapdel driver performs HDLC encoding and decoding on the
        fxogs           = 0      # Channel(s) are signalled using FXO Groundstart protocol.
        fxoks           = 0      # Channel(s) are signalled using FXO Koolstart protocol.
        fxols           = 0      # Channel(s) are signalled using FXO Loopstart protocol.
        fxsgs           = 0      # Channel(s) are signalled using FXS Groundstart protocol.
        fxsks           = 0      # Channel(s) are signalled using FXS Koolstart protocol.
        fxsls           = 0      # Channel(s) are signalled using FXS Loopstart protocol.
        indclear        = 0      # Like 'clear' except all channels are treated individually and
        nethdlc         = 0      # The zaptel driver bundles the channels together into an
        rawhdlc         = 0      # The zaptel driver performs HDLC encoding and decoding on the
        unused          = 0      # No signalling is performed, each channel in the list remains idle
    ...
    class add_dut_dut_flags(Enum):
        p_11r                 = 512      # Use .11r connection logic on all ssids, deprecated, see add_dut_ssid.
        AP_MODE               = 2        # (2) DUT acts as AP.
        DHCPD_LAN             = 64       # Provides DHCP server on LAN port
        DHCPD_WAN             = 128      # Provides DHCP server on WAN port
        EAP_PEAP              = 2048     # Use EAP-PEAP connection logic on all ssids, deprecated, see add_dut_ssid.
        EAP_TTLS              = 1024     # Use EAP-TTLS connection logic on all ssids, deprecated, see add_dut_ssid.
        INACTIVE              = 4        # (3) Ignore this in ChamberView, etc
        NOT_DHCPCD            = 4096     # Station/edge device that is NOT using DHCP.
        STA_MODE              = 1        # (1) DUT acts as Station.
        WEP                   = 8        # Use WEP encryption on all ssids, deprecated, see add_dut_ssid.
        ...
        WPA3                  = 256      # Use WPA3 encryption on all ssids, deprecated, see add_dut_extras.
    ...
    class add_dut_ssid_dut_flags(Enum):
        p_11r               = 512      # Use .11r connection logic
        EAP_PEAP            = 2048     # Use EAP-PEAP connection logic
        EAP_TTLS            = 1024     # Use EAP-TTLS connection logic
        WEP                 = 8        # Use WEP encryption
        WPA                 = 16       # Use WPA encryption
        WPA2                = 32       # Use WPA2 encryption
        WPA3                = 256      # Use WPA3 encryption
    ...
    class add_endp_payload_pattern(Enum):
        PRBS_11_8_10                  = 0      # PRBS (see above)
        PRBS_15_0_14                  = 0      # PRBS (see above)
        PRBS_4_0_3                    = 0      # Use linear feedback shift register to generate pseudo random sequence.
        PRBS_7_0_6                    = 0      # PRBS (see above)
        custom                        = 0      # Enter your own payload with the set_endp_payload cmd.
        decreasing                    = 0      # bytes start at FF and decrease, wrapping if needed
        increasing                    = 0      # bytes start at 00 and increase, wrapping if needed
        ones                          = 0      # payload is all ones  (FF)
        random                        = 0      # generate a new random payload each time sent
        random_fixed                  = 0      # means generate one random payload, and send it over and over again.
        zeros                         = 0      # payload is all zeros (00)

    class add_endp_type(Enum):
        custom_ether                  = 0      # LF frames with custom options, use with playback
        custom_mc_udp                 = 0      # LF Multicast UDP IPv4
        custom_tcp                    = 0      # LF TCP IPv4 frame with custom options
        custom_udp                    = 0      # LF UDP IPv4 frame with custom options
        lf                            = 0      # LF protocol
        lf_sctp                       = 0      # SCTP IPv4 protocol
        lf_sctp6                      = 0      # SCTP IPv6 protocol
        lf_tcp                        = 0      # TCP IPv4 connection
        lf_tcp6                       = 0      # TCP IPv6 connection
        lf_udp                        = 0      # UDP IPv4 connection
        lf_udp6                       = 0      # UDP IPv6 connection
        mc_udp                        = 0      # LF Multicast IPv4
    ...
    class add_file_endp_fio_flags(Enum):
        AUTO_MOUNT                    = 2       # (2) Attempt to mount with the provided information if not already mounted.
        AUTO_UNMOUNT                  = 4       # (4)   Attempt to un-mount when stopping test.
        CHECK_MOUNT                   = 1       # (1) Attempt to verify NFS and SMB mounts match the configured values.
        O_APPEND                      = 512     # (512) Open files for writing with O_APPEND instead
        O_DIRECT                      = 8       # (8)   Open file with O_DIRECT flag, disables caching.  Must use block-size read/write calls.
        O_LARGEFILE                   = 32      # (32)  Open files with O_LARGEFILE.  This allows greater than 2GB files on 32-bit systems.
        UNLINK_BW                     = 16      # (16)  Unlink file before writing.  This works around issues with CIFS for some file-servers.
        UNMOUNT_FORCE                 = 64      # (64)  Use -f flag when calling umount
        UNMOUNT_LAZY                  = 128     # (128)  Use -l flag when calling umount
        USE_FSTATFS                   = 256     # (256) Use fstatfs system call to verify file-system type when opening files.
    ...
    class add_file_endp_payload_pattern(Enum):
        ...
        ones                          = 0       # Payload is all ones  (FF).
        random                        = 0       # generate a new random payload each time sent.
        random_fixed                  = 0       # Means generate one random payload, and send it over
        zeros                         = 0       # Payload is all zeros (00).
    ...
    class add_group_flags(Enum):
        group_total_rates        = 4      # Set rates as total for group.
    ...
    class add_l4_endp_http_auth_type(Enum):
        ...
        DIGEST                                   = 2        # Digest (MD5) authentication
        GSSNEGOTIATE                             = 4        # GSS authentication
        NTLM                                     = 8        # NTLM authentication

    class add_l4_endp_proxy_auth_type(Enum):
        BASIC                                    = 1        # 1    Basic authentication
        BIND_DNS                                 = 512      # 512  Make DNS requests go out endpoints Port.
        DIGEST                                   = 2        # 2    Digest (MD5) authentication
        DISABLE_EPSV                             = 4096     # 4096 Disable FTP EPSV option
        DISABLE_PASV                             = 2048     # 2048 Disable FTP PASV option (will use PORT command)
        GSSNEGOTIATE                             = 4        # 4    GSS authentication
        INCLUDE_HEADERS                          = 256      # 256  especially for IMAP
        NTLM                                     = 8        # 8    NTLM authentication
        USE_DEFLATE_COMPRESSION                  = 128      # 128  Use deflate compression
        USE_GZIP_COMPRESSION                     = 64       # 64   Use gzip compression
        USE_IPV6                                 = 1024     # 1024 Resolve URL is IPv6.  Will use IPv4 if not selected.
        USE_PROXY_CACHE                          = 32       # 32   Use proxy cache

    class add_l4_endp_type(Enum):
        l4_generic                               = 0        # Layer 4 type
    ...
    class add_monitor_flags(Enum):
        disable_ht40        = 2048           # Disable HT-40 even if hardware and AP support it.
        disable_ht80        = 134217728      # Disable HT80 (for AC chipset NICs only)
        ht160_enable        = 4294967296     # Enable HT160 mode.
    ...
    class add_profile_profile_flags(Enum):
        p_11r                         = 64       # Use 802.11r roaming setup.
        BSS_TRANS                     = 1024     # Enable BSS Transition logic
        DHCP_SERVER                   = 1        # This should provide DHCP server.
        EAP_PEAP                      = 512      # Enable EAP-PEAP
        EAP_TTLS                      = 128      # Use 802.1x EAP-TTLS
        NAT                           = 256      # Enable NAT if this object is in a virtual router
        SKIP_DHCP_ROAM                = 16       # Ask station to not re-do DHCP on roam.
        WEP                           = 2        # Use WEP encryption
        WPA                           = 4        # Use WPA encryption
        WPA2                          = 8        # Use WPA2 encryption
        WPA3                          = 32       # Use WPA3 encryption
    ...
    class add_sta_flags(Enum):
        p_80211r_pmska_cache       = 67108864           # Enable oportunistic PMSKA caching for WPA2 (Related to 802.11r).
        p_80211u_additional        = 1048576            # AP requires additional step for access (802.11u Interworking)
        p_80211u_auto              = 262144             # Enable 802.11u (Interworking) Auto-internetworking feature.  Always enabled currently.
        p_80211u_e911              = 2097152            # AP claims emergency services reachable (802.11u Interworking)
        p_80211u_e911_unauth       = 4194304            # AP provides Unauthenticated emergency services (802.11u Interworking)
        p_80211u_enable            = 131072             # Enable 802.11u (Interworking) feature.
        p_80211u_gw                = 524288             # AP Provides access to internet (802.11u Interworking)
        p_8021x_radius             = 33554432           # Use 802.1x (RADIUS for AP).
        create_admin_down          = 68719476736        # Station should be created admin-down.
        custom_conf                = 32                 # Use Custom wpa_supplicant config file.
        disable_twt                = 17592186044416     # Disable TWT mode
        disable_fast_reauth        = 8589934592         # Disable fast_reauth option for virtual stations.
        disable_gdaf               = 16777216           # AP:  Disable DGAF (used by HotSpot 2.0).
        disable_ht80               = 134217728          # Disable HT80 (for AC chipset NICs only)
        disable_roam               = 2147483648         # Disable automatic station roaming based on scan results.
        disable_sgi                = 16384              # Disable SGI (Short Guard Interval).
        hs20_enable                = 8388608            # Enable Hotspot 2.0 (HS20) feature.  Requires WPA-2.
        ht160_enable               = 4294967296         # Enable HT160 mode.
        ht40_disable               = 2048               # Disable HT-40 even if hardware and AP support it.
        ibss_mode                  = 536870912          # Station should be in IBSS mode.
        lf_sta_migrate             = 32768              # OK-To-Migrate (Allow station migration between LANforge radios)
        mesh_mode                  = 17179869184        # Station should be in MESH mode.
        no_supp_op_class_ie        = 274877906944       # Do not include supported-oper-class-IE in assoc requests.  May work around AP bugs.
        osen_enable                = 1073741824         # Enable OSEN protocol (OSU Server-only Authentication)
        passive_scan               = 8192               # Use passive scanning (don't send probe requests).
        power_save_enable          = 34359738368        # Station should enable power-save.  May not work in all drivers/configurations.
        scan_ssid                  = 4096               # Enable SCAN-SSID flag in wpa_supplicant.
        txo_enable                 = 549755813888       # Enable/disable tx-offloads, typically managed by set_wifi_txo command
        use_bss_transition         = 8796093022208      # Enable BSS transition.
        use_wpa3                   = 1099511627776      # Enable WPA-3 (SAE Personal) mode.
        verbose                    = 65536              # Verbose-Debug:  Increase debug info in wpa-supplicant and hostapd logs.
        wds_mode                   = 137438953472       # WDS station (sort of like a lame mesh), not supported on ath10k
        wep_enable                 = 512                # Use wpa_supplicant configured for WEP encryption.
        wpa2_enable                = 1024               # Use wpa_supplicant configured for WPA2 encryption.
        wpa_enable                 = 16                 # Enable WPA
    ...
    class add_sta_rate(Enum):
        _a_g                       = 0                  # 6 Mbps, 9 Mbps, 12 Mbps, 18 Mbps, 24 Mbps, 36 Mbps, 48 Mbps, 54 Mbps
        _b                         = 0                  # 1Mbps, 2Mbps, 5.5 Mbps, 11 Mbps
        DEFAULT                    = 0                  # Use maximum available speed
        MCS0_76                    = 0                  # /n rates
        _bitmap_                   = 0                  # <b>'0xff 00 ...'</b> to directly specify the MCS bitmap.
    ...
    class add_traffic_profile_traffic_profile_flags(Enum):
        BI_DIRECTIONAL                        = 2      # Should we do bi-directional traffic?
        IPERF_UDP                             = 4      # If Iperf, should use UDP.  If not set, then will use TCP.
        UP                                    = 1      # Upload direction (this not set means download)

    class add_traffic_profile_wifi_mode(Enum):
        Iperf3_Client                         = 6      # iperf3 client
        Iperf3_Server                         = 5      # iperf3 server
        as_is                                 = 0      # Make no changes to current configuration
    ...
    class add_vap_flags(Enum):
        p_80211h_enable           = 268435456         # Enable 802.11h (needed for running on DFS channels)  Requires 802.11d.
        p_80211r_pmska_cache      = 67108864          # Enable oportunistic PMSKA caching for WPA2 (Related to 802.11r).
        p_80211u_additional       = 1048576           # AP requires additional step for access (802.11u Interworking)
        p_80211u_auto             = 262144            # Enable 802.11u (Interworking) Auto-internetworking feature.  Always enabled currently.
        p_80211u_e911             = 2097152           # AP claims emergency services reachable (802.11u Interworking)
        ...
        p_8021x_radius            = 33554432          # Use 802.1x (RADIUS for AP).
        create_admin_down         = 68719476736       # Station should be created admin-down.
        disable_dgaf              = 16777216          # AP Disable DGAF (used by HotSpot 2.0).
        disable_ht40              = 2048              # Disable HT-40 (will use HT-20 if available).
        disable_ht80              = 134217728         # Disable HT80 (for AC chipset NICs only)
        enable_80211d             = 64                # Enable 802.11D to broadcast country-code &amp; channels in VAPs
        enable_wpa                = 16                # Enable WPA
        hostapd_config            = 32                # Use Custom hostapd config file.
        hs20_enable               = 8388608           # Enable Hotspot 2.0 (HS20) feature.  Requires WPA-2.
        ht160_enable              = 4294967296        # Enable HT160 mode.
        osen_enable               = 1073741824        # Enable OSEN protocol (OSU Server-only Authentication)
        pri_sec_ch_enable         = 256               # Enable Primary/Secondary channel switch.
        short_preamble            = 128               # Allow short-preamble
        use_bss_load              = 2199023255552     # Enable BSS Load IE in Beacons and Probe Responses (.11e).
        use_bss_transition        = 8796093022208     # Enable BSS transition.
        use_rrm_report            = 4398046511104     # Enable Radio measurements IE in beacon and probe responses.
        use_wpa3                  = 1099511627776     # Enable WPA-3 (SAE Personal) mode.
        verbose                   = 65536             # Verbose-Debug:  Increase debug info in wpa-supplicant and hostapd logs.
        wep_enable                = 512               # Enable WEP Encryption
        wpa2_enable               = 1024              # Enable WPA2 Encryption
    ...
    class add_venue_freq_5(Enum):
        Ch_100         = 2048         # Channel 100 5500
        Ch_104         = 4096         # Channel 104 5520
        Ch_108         = 8192         # Channel 108 5540
        Ch_112         = 16384        # Channel 112 5560
        Ch_116         = 32768        # Channel 116 5580
        Ch_120         = 65536        # Channel 120 5600
        Ch_124         = 131072       # Channel 124 5620
        Ch_128         = 262144       # Channel 128 5640
        Ch_132         = 524288       # Channel 132 5660
        Ch_136         = 1048576      # Channel 136 5680
        Ch_140         = 2097152      # Channel 140 5700
        Ch_149         = 4194304      # Channel 149 5745
        Ch_153         = 8388608      # Channel 153 5765
        Ch_157         = 16777216     # Channel 157 5785
        Ch_161         = 33554432     # Channel 161 5805
        Ch_165         = 67108864     # Channel 165 5825
        Ch_36          = 1            # Channel 36  5180
        Ch_38          = 2            # Channel 38  5190
        Ch_40          = 4            # Channel 40  5200
        Ch_42          = 8            # Channel 42  5210
        Ch_44          = 16           # Channel 44  5220
        Ch_46          = 32           # Channel 46  5230
        Ch_48          = 64           # Channel 48  5240
        Ch_52          = 128          # Channel 52  5260
        Ch_56          = 256          # Channel 56  5280
        Ch_60          = 512          # Channel 60  5300
        Ch_64          = 1024         # Channel 64  5320
    ...
    class add_vr_flags(Enum):
        p_4BYTE_AS_NUMBER      = 64       # Sets corresponding Xorp flag.
        BGP_CONFED             = 256      # Configure BGP in a confederation.
        BGP_DAMPING            = 512      # Enable BGP damping section in Xorp configuration file.
        ENABLE_BGP             = 32       # Set this to zero if you don't want BGP on this VR.
        RIP_ACCEPT_DR          = 2048     # Tell RIP to accept default-routes.
        ROUTE_REFLECTOR        = 128      # Act as BGP Route Reflector.
        USE_IPV6               = 16       # Enable IPv6 OSPF routing for this virtual router.
        USE_IPV6_RADVD         = 8        # Enable IPv6 RADV Daemon for interfaces in this virtual router.
        USE_RIP                = 1024     # Enable RIP routing protocol in Xorp.
        USE_XORP_MCAST         = 2        # Enable Xorp Multicast routing (requires OSPF to be enabled currently)
        USE_XORP_OLSR          = 4096     # Enable OLSR routing protocol in Xorp.
        USE_XORP_OSPF          = 1        # Enable Xorp router daemon with OSPF (IPv4) protocol
        USE_XORP_SHA           = 4        # Enable Telcordia's Xorp SHA option (requires OSPF to be enabled)
    ...
    class add_vrcx_flags(Enum):
        custom_dhcpd         = 1024     # Use custom DHCP config file
        dhcpd_enabled        = 512      # Serve IPv4 DHCP on this interface
        ipv6_enabled         = 8192     # Serve IPv6 DHCP on this interface
        nat_enabled          = 256      # This connection will NAT outgoing packets
        subnet_0             = 1        # Specify subnet 0
        subnet_1             = 2        # Specify subnet 1
        subnet_2             = 4        # Specify subnet 2
        subnet_3             = 8        # Specify subnet 3
        subnet_4             = 16       # Specify subnet 4
        subnet_5             = 32       # Specify subnet 5
        subnet_6             = 64       # Specify subnet 6
        subnet_7             = 128      # Specify subnet 7
        use_multicast        = 2048     # Use this interface for multicast and-rp
        use_vrrp             = 4096     # Use this interface for VRRP
    ...
    class c_show_ports_probe_flags(Enum):
        BRIDGE                        = 8      # 8 include bridges
        EASY_IP_INFO                  = 16     # 16 Everything but gateway information, which is expensive to probe.
        ETHTOOL                       = 4      # 4 include ethtool results
        GW                            = 32     # 32 include gateway information
        GW_FORCE_REFRESH              = 64     # 64 Force GW (re)probe.  Otherwise, cached values *might* be used.
        MII                           = 2      # 2 include MII
        WIFI                          = 1      # 1 include wifi stations
    ...
    class clear_port_counters_extra(Enum):
        dhcp4_lease        = 0      # Remove dhcp lease files for IPv4 DHCP
        dhcp6_lease        = 0      # Remove dhcp lease files for IPv6 DHCP
        dhcp_leases        = 0      # Remove dhcp lease files for IPv4 and IPv6 DHCP
    ...
    class log_level_level(Enum):
        ALL            = 4294967295     # Log everything
        CUST1          = 65536          # Cust-1, latency info     (65536)
        DB             = 128            # Database related logging (128)
        DBG            = 32             # debug                    (32)
        DBG2           = 4096           # very verbose logging     (4096)
        DIS            = 1              # disasters                (1)
        ERR            = 2              # errors                   (2)
        INF            = 8              # info                     (8)
        LIO            = 8192           # IO logging               (8192)
        LL_PROF        = 32768          # Profiling information    (32768)
        OUT1           = 16384          # Some std-out logging     (16384)
        PARSE          = 2048           # PARSE specific           (2048)
        SCRIPT         = 1024           # Scripting specific stuff (1024)
        SEC            = 64             # log security violations  (64)
        TRC            = 16             # function trace           (16)
        WRN            = 4              # warnings                 (4)
        XMT            = 256            # Output going to clients  (256)
    ...
    class reset_port_pre_ifdown(Enum):
        P_IN              = 0      # Only call the portal login (do not reset drivers/supplicant/dhcp)
        P_OUT             = 0      # Only call the portal logout (do not reset drivers/supplicant/dhcp)
        YES               = 0      # (include logout) Call portal-bot.pl ... <b>--logout</b> before going down.
    ...
    class scan_wifi_extra(Enum):
        NA                         = 0      # (or left blank) the system does a full scan
        dump                       = 0      # then only cached values are returned
        trigger_freq__freq_        = 0      # scan exactly those frequencies
    ...
    class set_arm_info_arm_flags(Enum):
        random_payload            = 65536     # Use random payload sizes instead of linear increase
        rel_tstamp                = 1024      # Use Relative Timestamps.  This will increase performance
        slow_start                = 8192      # Use slow-start logic.  This ramps up
        udp_checksum              = 16384     # Use UDP Checksums.
        use_gw_mac                = 4096      # Use default gateway's MAC for destination MAC.
        use_tcp                   = 32768     # Use TCP instead of UDP protocol. (Note this is NOT stateful TCP!)
    ...
    class set_cx_state_cx_state(Enum):
        DELETED           = 0      # Deletes the CX(s).
        QUIESCE           = 0      # Stop transmitting and gracefully stop cross-connect.
        RUNNING           = 0      # Sets the CX(s) in the running state.
        STOPPED           = 0      # Sets the CX(s) in the stopped state.
        SWITCH            = 0      # Sets the CX(s) in the running state, stopping any conflicting tests.
    ...
    class set_endp_flag_flag(Enum):
        AutoHelper                  = 0      # Automatically run on helper process
        ClearPortOnStart            = 0      # clear stats on start
        DoChecksum                  = 0      # Enable checksumming
        EnableConcurrentSrcIP       = 0      # Concurrent source IPs?
        EnableLinearSrcIP           = 0      # linearized source IPs
        EnableLinearSrcIPPort       = 0      # linearized IP ports
        EnableRndSrcIP              = 0      # randomize source IP
        KernelMode                  = 0      # Enable kernel mode
        QuiesceAfterDuration        = 0      # quiesce after time period
        QuiesceAfterRange           = 0      # quiesce after range of bytes
        Unmanaged                   = 0      # Set endpoint unmanaged
        UseAutoNAT                  = 0      # NAT friendly behavior
    ...
    class set_endp_payload_payload_type(Enum):
        ...
        PRBS_4_0_3                 = 0      # Use linear feedback shift register to generate pseudo random sequence.
        PRBS_7_0_6                 = 0      # PRBS (see above)
        custom                     = 0      # Enter your own payload with the set_endp_payload
        decreasing                 = 0      # bytes start at FF and decrease, wrapping if needed.
        increasing                 = 0      # bytes start at 00 and increase, wrapping if needed.
        ones                       = 0      # Payload is all ones  (FF).
        random                     = 0      # generate a new random payload each time sent.
    ...
    class set_event_interest_ei_flags(Enum):
        ...
        SET                             = 1              # set interest flag

    class set_event_interest_events1(Enum):
        BAD_TOS                         = 4194304        # Endpoint has bad ToS values configured.
        Bad_MAC                         = 1048576        # Invalid MAC address configured.
        Cleared                         = 8192           # Counters were cleared for some entity.
        Connect                         = 256            # WiFi interface connected to AP.
        Custom                          = 4              # Custom event (generated by USER in GUI or CLI).
        DHCP_Fail                       = 32768          # DHCP Failed, maybe out of leases?
        DHCP_Timeout                    = 65536          # Timed out talking to DHCP server.
        DHCP4_Error                     = 131072         # DHCP gave out duplicated IP address.
        DHCP6_Error                     = 262144         # DHCPv6 gave out duplicated IPv6 address.
        Disconnect                      = 128            # WiFi interface disconnected from AP.
        Endp_Started                    = 64             # Endpoint was started.
        Endp_Stopped                    = 32             # Endpoint stopped for some reason.
        Link_Down                       = 1              # Notify when Interface Link goes DOWN.
        Link_Errors                     = 16384          # Port shows low-level link errors.
        Link_Up                         = 2              # Notify when Interface Link goes UP.
        Login                           = 1024           # CLI/GUI user connected to LANforge.
        Logout                          = 512            # CLI/GUI user disconnected from LANforge.
        Migrated                        = 2097152        # Port (station network interface) migrated.
        NO_RX_SINCE                     = 8388608        # Endpoint threshold alert.
        NO_RX_SINCE_CLEARED             = 16777216       # Endpoint threshold alert cleared.
        RX_BPS_OOR_1M                   = 536870912      # Endpoint threshold alert.
        RX_BPS_OOR_1M_CLEARED           = 1073741824     # Endpoint threshold alert cleared.
        RX_BPS_OOR_30S                  = 134217728      # Endpoint threshold alert.
        RX_BPS_OOR_30S_CLEARED          = 268435456      # Endpoint threshold alert cleared.
        RX_BPS_OOR_3S                   = 33554432       # Endpoint threshold alert.
        RX_BPS_OOR_3S_CLEARED           = 67108864       # Endpoint threshold alert cleared.
        Resource_Down                   = 8              # Resource has crashed, rebooted, etc.
        Resource_Up                     = 16             # Resource has connected to manager.
        Start_Reports                   = 4096           # Start saving report data files (CSV).
        Stop_Reports                    = 2048           # Stop saving report data files (CSV).
        TX_BPS_OOR_3S                   = 2147483648     # Endpoint threshold alert.
        WiFi_Config                     = 524288         # WiFi Configuration Error.

    class set_event_interest_events2(Enum):
        FW_CRASH                        = 2048           # Firmware for entity has crashed.
        FW_FAIL                         = 4096           # Firmware failed powerup, may require reboot.
        IFDOWN_FAIL                     = 32768          # IFDOWN-PRE Script (ifup --logout) returned error code.
        IFDOWN_OK                       = 65536          # IFDOWN-PRE Script (ifup --logout) completed successfully.
        IFUP_FAIL                       = 8192           # IFUP-POST Script returned error code.
        IFUP_OK                         = 16384          # IFUP-POST Script completed successfully.
        RX_DROP_OOR_1M                  = 512            # Endpoint threshold alert.
        RX_DROP_OOR_1M_CLEARED          = 1024           # Endpoint threshold alert cleared.
        RX_DROP_OOR_3S                  = 128            # Endpoint threshold alert.
        RX_DROP_OOR_3S_CLEARED          = 256            # Endpoint threshold alert cleared.
        RX_LAT_OOR                      = 32             # Endpoint threshold alert.
        RX_LAT_OOR_CLEARED              = 64             # Endpoint threshold alert cleared.
        TX_BPS_OOR_1M                   = 8              # Endpoint threshold alert.
        TX_BPS_OOR_1M_CLEARED           = 16             # Endpoint threshold alert cleared.
        TX_BPS_OOR_30S                  = 2              # Endpoint threshold alert.
        TX_BPS_OOR_30S_CLEARED          = 4              # Endpoint threshold alert cleared.
    ...
    class set_flag_flag(Enum):
        brief                    = 0      # Request more abbreviated output to various commands.
        prompt_newlines          = 0      # Add a newline after every prompt.  Can help with scripts
        push_all_rpts            = 0      # If enabled, server will send port, endpoint, and other
        push_endp_rpts           = 0      # If enabled, server will send endpoint reports without
        request_keyed_text       = 0      # Normally most keyed-text events are only sent to the GUI
        stream_events            = 0      # Normally the CLI will not show Events (as seen in the Event
    ...
    class set_port_cmd_flags(Enum):
        abort_if_scripts                  = 1024                   # Forceably abort all ifup/down scripts on this Port.
        force_MII_probe                   = 4                      # Force MII probe
        from_dhcp                         = 512                    # Settings come from DHCP client.
        from_user                         = 128                    # from_user (Required to change Mgt Port config
        new_gw_probe                      = 32                     # Force new GW probe
        new_gw_probe_dev                  = 64                     # Force new GW probe for ONLY this interface
        no_hw_probe                       = 8                      # Don&apos;t probe hardware
        probe_wifi                        = 16                     # Probe WIFI
        reset_transceiver                 = 1                      # Reset transciever
        restart_link_neg                  = 2                      # Restart link negotiation
        skip_port_bounce                  = 256                    # skip-port-bounce  (Don&apos;t ifdown/up
        use_pre_ifdown                    = 2048                   # Call pre-ifdown script before bringing interface down.

    class set_port_current_flags(Enum):
        adv_100bt_fd                      = 8388608                # advert-100bt-FD
        adv_100bt_hd                      = 4194304                # advert-100bt-HD
        adv_10bt_fd                       = 2097152                # advert-10bt-FD
        adv_10bt_hd                       = 1048576                # advert-10bt-HD
        adv_10g_fd                        = 34359738368            # advert-10G-FD
        adv_2_5g_fd                       = 17179869184            # advert-2.5G-FD
        adv_5g_fd                         = 288230376151711744     # Advertise 5Gbps link speed.
        adv_flow_ctl                      = 134217728              # advert-flow-control
        auto_neg                          = 256                    # auto-negotiate
        aux_mgt                           = 140737488355328        # Enable Auxillary-Management flag for this port.
        fixed_100bt_fd                    = 16                     # Fixed-100bt-FD
        fixed_100bt_hd                    = 8                      # Fixed-100bt-HD
        fixed_10bt_fd                     = 4                      # Fixed-10bt-FD
        fixed_10bt_hd                     = 2                      # Fixed-10bt-HD (half duplex)
        ftp_enabled                       = 70368744177664         # Enable FTP (vsftpd) service for this port.
        gro_enabled                       = 274877906944           # GRO-Enabled
        gso_enabled                       = 1099511627776          # GSO-Enabled
        http_enabled                      = 35184372088832         # Enable HTTP (nginx) service for this port.
        if_down                           = 1                      # Interface Down
        ignore_dhcp                       = 562949953421312        # Don&apos;t set DHCP acquired IP on interface,
        ipsec_client                      = 18014398509481984      # Enable client IPSEC xfrm on this port.
        ipsec_concentrator                = 36028797018963968      # Enable concentrator (upstream) IPSEC xfrm on this port.
        lro_enabled                       = 137438953472           # LRO-Enabled
        no_dhcp_rel                       = 8796093022208          # No-DHCP-Release
        no_dhcp_restart                   = 281474976710656        # Disable restart of DHCP on link connect (ie, wifi).
        no_ifup_post                      = 1125899906842624       # Skip ifup-post script if we can detect that we
        promisc                           = 268435456              # PROMISC
        radius_enabled                    = 9007199254740992       # Enable RADIUS service (using hostapd as radius server)
        rxfcs                             = 4398046511104          # RXFCS
        service_dns                       = 72057594037927936      # Enable DNS (dnsmasq) service on this port.
        staged_ifup                       = 17592186044416         # Staged-IFUP
        tso_enabled                       = 68719476736            # TSO-Enabled
        ufo_enabled                       = 549755813888           # UFO-Enabled
        use_dhcp                          = 2147483648             # USE-DHCP
        use_dhcpv6                        = 2199023255552          # USE-DHCPv6

    class set_port_dhcp_client_id(Enum):
        NA                                = 0                      # Do not change from current value.
        NONE                              = 0                      # Do not use dhcp client ID.
        _string_                          = 0                      # Use the string for the client ID.
        __DEVNAME                         = 0                      # Use the interface&apos;s name as the client ID.
        __MAC                             = 0                      # Use interface&apos;s MAC address for the client ID.

    class set_port_dhcp_hostname(Enum):
        NA                                = 0                      # Do not change from current value.
        NONE                              = 0                      # Do not use dhcp Hostname
        _string_                          = 0                      # Use the string for the Hostname.
        __ALIAS__                         = 0                      # Use alias if set, or EID behaviour if alias is not set..
        __EID__                           = 0                      # Use hostname 'CT-[resource-id].[port-name]'

    class set_port_dhcp_vendor_id(Enum):
        NA                                = 0                      # Do not change from current value.
        NONE                              = 0                      # Do not use dhcp vendor ID
        _string_                          = 0                      # Use the string for the vendor ID.

    class set_port_flags2(Enum):
        bypass_disconnect                 = 512                    # Logically disconnect the cable (link-down)
        bypass_enabled                    = 16                     # Enable Bypass Device
        bypass_power_down                 = 128                    # Should bypass be on when we shutdown or loose power?
        bypass_power_on                   = 256                    # Should bypass be on when we first power up?
        supports_bypass                   = 2                      # Support Bypass Devices
        use_stp                           = 1                      # Use Spanning Tree Protocol

    class set_port_interest(Enum):
        alias                             = 4096                   # Port alias
        aux_mgt                           = 536870912              # Enable/disable Auxillary-Management for a port
        bridge                            = 65536                  # BRIDGE
        bypass                            = 262144                 # Bypass
        command_flags                     = 1                      # apply command flags
        cpu_mask                          = 1048576                # CPU Mask, useful for pinning process to CPU core
        current_flags                     = 2                      # apply current flags
        dhcp                              = 16384                  # including client-id.
        dhcp_rls                          = 67108864               # DHCP release
        dhcpv6                            = 16777216               # Use DHCPv6
        gen_offload                       = 524288                 # Generic offload flags, everything but LRO
        ifdown                            = 8388608                # Down interface
        interal_use_1                     = 2048                   # (INTERNAL USE)
        ip_Mask                           = 8                      # IP mask
        ip_address                        = 4                      # IP address
        ip_gateway                        = 16                     # IP gateway
        ipv6_addrs                        = 131072                 # IPv6 Address
        link_speed                        = 128                    # Link speed
        lro_offload                       = 2097152                # LRO (Must be disabled when used in Wanlink,
        mac_address                       = 32                     # MAC address
        mtu                               = 256                    # MTU
        no_apply_dhcp                     = 2147483648             # Enable/disable NO-APPLY-DHCP flag for a port
        no_dhcp_conn                      = 1073741824             # Enable/disable NO-DHCP-ON-CONNECT flag for a port
        promisc_mode                      = 1024                   # PROMISC mode
        rpt_timer                         = 32768                  # Report Timer
        rx_all                            = 8192                   # Rx-ALL
        rxfcs                             = 33554432               # RXFCS
        skip_ifup_roam                    = 4294967296             # Enable/disable SKIP-IFUP-ON-ROAM flag for a port
        sta_br_id                         = 4194304                # WiFi Bridge identifier.  0 means no bridging.
        supported_flags                   = 64                     # apply supported flags
        svc_ftpd                          = 268435456              # Enable/disable FTP Service for a port
        svc_httpd                         = 134217728              # Enable/disable HTTP Service for a port
        tx_queue_length                   = 512                    # TX Queue Length
    ...
    class set_rfgen_rfgen_flags(Enum):
        one_burst              = 8      # Run for about 1 second and stop.  Uses 5-sec sweep time for single pulse train.
        running                = 2      # Should we start the RF Generator or not?
    ...
    class set_script_flags(Enum):
        SCR_COMPLETED                = 128        # Set automatically by LANforge.
        SCR_HIDE_CONSTRAINTS         = 8192       # Hide constraints messages.
        SCR_HIDE_CSV                 = 32         # Don't print the CSV data in the report.
        SCR_HIDE_HUNT                = 2048       # Hide the individual hunt steps..just show results.
        SCR_HIDE_ITER_DETAILS        = 8          # Hide iteration detail reports.
        SCR_HIDE_LAT                 = 4096       # Hide latency distribution reports.
        SCR_HIDE_LEGEND              = 16         # Don't print the legend in the report.
        SCR_LOOP                     = 256        # Loop script until manually stopped.
        SCR_NO_KEYED_RPT             = 2          # Script should NOT send reports to the CLI/GUI.
        SCR_RUN_ON_MGR               = 64         # Set automatically by LANforge.
        SCR_SHOW_ATTENUATION         = 16384      # Show attenuation packet stats.
        SCR_SHOW_DUPS                = 512        # Report duplicate packets.
        SCR_SHOW_GOLDEN_3P           = 131072     # Add 'golden' third-party AP graph for comparison (where available).
        SCR_SHOW_GOLDEN_LF           = 65536      # Add 'golden' LANforge graph for comparison (where available).
        SCR_SHOW_OOO                 = 1024       # Report out-of-order packets.
        SCR_STOPPED                  = 1          # Script should NOT have any affect on the endpoint.
        SCR_SYMMETRIC                = 4          # This script should apply settings to the peer endpoing as well.
        SCR_USE_MSS                  = 32768      # When setting packet size, set TCP MSS instead if endpoint supports that.

    class set_script_type(Enum):
        NONE                         = 0          # Delete any existing script.
        Script2544                   = 0          # For RFC 2544 type testing.
        ScriptAtten                  = 0          # For Attenuators only.
        ScriptHunt                   = 0          # Hunt for maximum speed with constraints.
        ScriptWL                     = 0          # For iterating through WanLink settings
    ...
    class set_wanpath_running_running(Enum):
        AS_PARENT          = 0      # then it will be started and stopped as the parent WanLink is.
        RUNNING            = 0      # then it will be running at all times
        STOPPED            = 0      # then it will not be running at any time.
    ...
    class set_wifi_corruptions_corrupt_flags(Enum):
        MSG_TYPE_DEAUTH                          = 2        # de-authentication message
        MSG_TYPE_EAPOL                           = 1        # Any EAPOL message
        MSG_TYPE_EAPOL_1_OF_2                    = 64       # EAPOL message 1/2
        MSG_TYPE_EAPOL_1_OF_4                    = 4        # EAPOL message 1/4
        MSG_TYPE_EAPOL_2_OF_2                    = 128      # EAPOL message 2/2
        MSG_TYPE_EAPOL_2_OF_4                    = 8        # EAPOL message 2/4
        MSG_TYPE_EAPOL_3_OF_4                    = 16       # EAPOL message 3/4
        MSG_TYPE_EAPOL_4_OF_4                    = 32       # EAPOL message 4/4
        MSG_TYPE_EAPOL_ASSOC                     = 512      # EAP Association
        MSG_TYPE_EAPOL_KEY_REQ                   = 256      # EAP Key Request (not sure if this works properly)
        MST_TYPE_EAPOL_ID_REQ                    = 1024     # EAP Identity request
        MST_TYPE_EAPOL_ID_RESP                   = 2048     # EAP Identity response
        MST_TYPE_EAPOL_OTHER_REQ                 = 4096     # EAP Requests that do not match other things.
        MST_TYPE_EAPOL_OTHER_RESP                = 8192     # EAP Responses that do not match other things.
    ...
    class set_wifi_radio_flags(Enum):
        ct_sta_mode          = 262144        # Enable CT-STA mode if radio supports it.  Efficiently replaces sw-crypt in some firmware.
        firmware_cfg         = 524288        # Apply firmware config.
        hw_sim               = 1             # Create hw-sim virtual radio if radio does not already exist.
        ignore_radar         = 1048576       # Ignore RADAR events reported by firmware.
        no_scan_share        = 64            # Disable sharing scan results.
        no_sw_crypt          = 131072        # Disable software-crypt for this radio.  Disables some virtual-station features.
        use_syslog           = 536870912     # Put supplicant logs in syslog instead of a file.
        verbose              = 65536         # Verbose-Debug:  Increase debug info in wpa-supplicant and hostapd logs.
    ...
    class set_wl_qdisc_qdisc(Enum):
        FIFO                         = 0      # is the default queuing discipline, no arguments
        WRR__queue_queue_____        = 0      # Weighted Round Robbin is also available
    ...
    class sniff_port_flags(Enum):
        DUMPCAP              = 2      # Use command-line dumpcap, more efficient than tshark
        MATE_TERMINAL        = 4      # Launch tshark/dumpcap in mate-terminal
        MATE_XTERM           = 8      # Launch tshark/dumpcap in xterm
        TSHARK               = 1      # Use command-line tshark instead of wireshark
